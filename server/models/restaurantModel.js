...
const Restaurant = {
    createRestaurant: (ownerId, name, description, location, image) => {
        return new Promise((resolve, reject) => {
            const sql = `
                ...
                VALUES (?, ?, ?, ?, ?)
            `;
            db.query(sql, [ownerId, name, description, location, image], (err, result) => {
                if (err) reject(err);
                else resolve(result);
            });
        });
    ...
    uploadMultipleRestaurantImages: (imageEntries) => {
        return new Promise((resolve, reject) => {
            const sql = `
                ...
                VALUES ?
            `;
            const values = imageEntries.map(entry => [entry.restaurant_id, entry.image_url, entry.description, entry.created_at]);
    ...
    getRestaurantImagesById: (restaurantId) => {
        return new Promise((resolve, reject) => {
            db.query('SELECT * FROM restaurant_images WHERE restaurant_id = ?', [restaurantId], (err, result) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
